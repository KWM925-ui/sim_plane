...
DEFAULT_MATRIX_PATH = REPO_ROOT / "configs" / "human_follow_stage2_integrated_acceptance_matrix.json"
DEFAULT_REPORT_ROOT = REPO_ROOT / "runs" / "human_follow_stage2_integrated_acceptance"
DEFAULT_KEEP_LAST = 5
...
def validate_matrix(path=None, artifact_root=None, use_latest=False):
    ...
    return {
        "matrix_name": matrix.get("matrix_name", "human_follow_stage2_integrated_acceptance"),
...
def format_report(report):
    lines = []
    lines.append(
        "human-follow stage2 integrated acceptance: {0} ({1} artifacts)".format(
...
def write_report(report, report_root=None, keep_last=DEFAULT_KEEP_LAST):
    ...
    matrix_name = report.get("matrix_name", "human_follow_stage2_integrated_acceptance")
...
def build_delta(current_report, previous_report):
    ...
    for name in sorted(set(previous_rows) | set(current_rows)):
        ...
        for metric_name in (
            ...
            "max_altitude_m",
            "max_speed_mps",
            "algorithm_adapter_stage2_goal_count",
            "algorithm_adapter_stage2_ego_cmd_count",
            "algorithm_adapter_stage2_nonzero_mavros_setpoint_count",
            "algorithm_adapter_stage2_follow_goal_count",
            "algorithm_adapter_stage2_search_goal_count",
            "algorithm_adapter_stage2_hold_goal_count",
        ):
