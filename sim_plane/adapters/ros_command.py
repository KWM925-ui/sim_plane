...
from sim_plane.adapters.external_command import build_context_env, load_result_payload, resolve_path, shutil_which
...
class ROSCommandAdapter(AlgorithmAdapter):
    ...
    def run(self, spec, sink, context):
        ...
        wait_for_ros_master(config["env"]["ROS_MASTER_URI"], config["master_timeout_s"])
...
def wait_for_ros_master(master_uri, timeout_s):
...
def determine_success(
    ...
    if timed_out and not allow_timeout_as_success:
        return False
    if stop_requested and treat_stop_request_as_success:
        return exit_code in success_exit_codes or exit_code in (-signal.SIGINT, 128 + signal.SIGINT)
