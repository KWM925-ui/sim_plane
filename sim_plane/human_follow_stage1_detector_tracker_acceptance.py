...
DEFAULT_MATRIX_PATH = REPO_ROOT / "configs" / "human_follow_stage1_detector_tracker_acceptance_matrix.json"
DEFAULT_REPORT_ROOT = REPO_ROOT / "runs" / "human_follow_stage1_detector_tracker_acceptance"
DEFAULT_KEEP_LAST = 5
...
def validate_matrix(path=None, artifact_root=None, use_latest=False):
    ...
    return {
        "matrix_name": matrix.get("matrix_name", "human_follow_stage1_detector_tracker_acceptance"),
...
def format_report(report):
    lines = []
    lines.append(
        "human-follow stage1 detector-tracker acceptance: {0} ({1} artifacts)".format(
...
def write_report(report, report_root=None, keep_last=DEFAULT_KEEP_LAST):
    ...
    matrix_name = report.get("matrix_name", "human_follow_stage1_detector_tracker_acceptance")
...
def build_delta(current_report, previous_report):
    ...
    for name in sorted(set(previous_rows) | set(current_rows)):
        ...
        for metric_name in (
            ...
            "algorithm_adapter_setpoint_count",
