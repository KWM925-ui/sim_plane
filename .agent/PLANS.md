...
## Paper-Grade Evaluation Frontier 2026-06-01

- External review feedback was accepted as a platform-mainline correction, not
  as marketing text:
  - the platform should be described as a UAV algorithm simulation/evaluation
    platform, not a high-fidelity visual simulator;
  - the next value layer is a fixed paper/project-style experiment loop,
    baseline algorithm entrypoints, and clearer sensor data-stream fault tests;
  - PX4-native failure injection remains separate from lightweight demo
    sensor-stream degradation.
- Scope boundaries:
  - do not touch `/home/coco/follwer_ws`;
  - do not modify the existing dirty human-follow/Gazebo Stage2 files in this
    workspace;
  - do not claim AirSim/Isaac/Flightmare/FlightGoggles-grade visual realism;
  - do not call demo `sensor_stream_faults` PX4-native failure injection;
  - do not advertise planned baselines as runnable.
- Landed target:
  - `python3 -m sim_plane quadrotor-exam --artifact-root runs`;
  - `configs/paper_quadrotor_exam_suite.json`;
  - `configs/demo_sensor_stream_fault_suite.json`;
  - `python3 -m sim_plane list-baselines`;
  - `python3 -m sim_plane run-baseline <name>`.
- Fresh closure evidence:
  ...
  - `git diff --check`: PASS;
  - `python3 -m sim_plane doctor --json`: PASS-level readiness, `12` ready backends and `6` ready adapters;
  ...
  - `python3 -m sim_plane artifact-hygiene --artifact-root runs --json`: PASS, `status=clean`, `complete_artifact_count=321`, `attention_count=0`;
  - `python3 -m sim_plane manual-probe-hygiene --artifact-root runs --json`: PASS, `status=clean`.
- Hygiene correction:
  - the temporary debug artifact root created during failure diagnosis was pruned after removing its accidental exact-path mention from supervisor notes;
  - `__pycache__` / `*.pyc` generated by validation were removed after the final runs.
  - `imu_noise_burst.speed_std_mps` now actually perturbs demo speed telemetry during the burst window instead of being an inert config field.

## Platform Health Frontier 2026-06-01

- Current frontier: add one system-level health entrypoint before widening
  another functional surface.
- Why this comes first:
  - the platform now has many separate evidence surfaces, so the next systemic
    weakness is state fragmentation rather than lack of another backend;
  - every future optimization should start from a single command that says
    which surfaces are green, which hygiene checks are clean, and what objective
    boundaries still exist.
- Landed target:
  - `python3 -m sim_plane platform-health --artifact-root runs`;
  - report root `runs/platform_health/`;
  - artifact hygiene now treats `platform_health` as a reserved report root.
- Scope boundaries:
  - do not touch `/home/coco/follwer_ws`;
  - do not reopen human-follow Stage1/Stage2;
  - do not change existing acceptance matrices, thresholds, or semantics;
  - do not call demo degradation/fuzz or `sensor_stream_faults` PX4-native
    failure injection.
- The health report aggregates:
  - git worktree state;
  - `doctor` readiness;
  - artifact hygiene and manual-probe hygiene;
  - latest platform, planner, PX4 failure, and quadrotor exam acceptance;
  - latest suite, fuzz, flight-log, ULog, autotest, and professional test
    surface summaries;
  - objective boundaries and a next-stage candidate plan.
- Next-stage candidate priorities are now explicit but not auto-executed:
  - PX4 `.ulg` auto-collection closure;
  - PX4-native failure expansion with fresh official evidence only;
  - dashboard health/report consolidation;
  - baseline catalog hardening.
