...
## ROS2 / Gazebo Migration Roadmap Closure 2026-05-27

### Locked Facts

- Active frontier is the generic `sim_plane` platform mainline.
...
- Gazebo Harmonic official binary support is centered on Ubuntu `22.04` and `24.04`, while the current validated host baseline is Ubuntu `20.04`.
- Current platform evidence depends on stable Ubuntu 20.04 paths including `PX4 SIH`, `PX4 + JSBSim`, `PX4 + Gazebo Classic`, `MARSIM`, `FAST_LIO + MARSIM`, and EGO-Planner-family ROS1 paths.

### Current Frontier

- Close the remaining platform-mainline long-term risk item by documenting a ROS2 / Gazebo Harmonic migration roadmap without changing runtime behavior or acceptance semantics.

### Forbidden Actions

- Do not change scenarios, backends, adapters, or acceptance thresholds for this roadmap.
- Do not touch `/home/coco/follwer_ws`.
- Do not replace the current Ubuntu 20.04 runtime line.
- Do not treat the roadmap as proof that the ROS2/Gazebo Harmonic backend is implemented.

### Closure

- Added `docs/ros2_gazebo_migration_roadmap_zh.md`.
- Added the roadmap link to `README.md`.
- Updated `.agent/PLANS.md` with the roadmap closure and future migration constraints.
- No runtime code, scenario behavior, acceptance matrix, or artifact semantics were changed.

### Newly Demoted This Round

- "The platform should immediately replace Gazebo Classic with Gazebo Harmonic on the current Ubuntu 20.04 host" is demoted.
- "ROS2 migration should be done by editing the existing ROS1/Gazebo Classic paths in place" is demoted.

### Only Question Next Round

- Which platform-mainline capability should be improved next based on a concrete user need, now that the known hygiene, onboarding, live-smoke, dashboard, and migration-roadmap gaps are covered?

### Forbidden Next Round

- No `/home/coco/follwer_ws` changes unless the user explicitly reopens that project branch.
- No Stage1/Stage2 human-follow acceptance work unless explicitly requested.
- No acceptance semantic or threshold changes without a fresh, narrowly scoped defect.
- No ROS2/Gazebo Harmonic implementation work unless the user explicitly chooses that migration frontier and an isolated target environment is available.

### Promotion Gate

- Roadmap-only change passed the required repository unit tests, Python compile, shell syntax checks, `doctor --json`, and artifact hygiene checks before commit/push.
...
- `bash -n scripts/*.sh`: passed.
- `python3 -m sim_plane doctor --json`: `12` ready backends, `5` ready adapters.
- `python3 -m sim_plane artifact-hygiene --artifact-root runs --json`: clean, `reserved_root_count=8`, `complete_artifact_count=112`, `attention_count=0`.
- Generated Python `__pycache__` residue from validation was removed before commit.
